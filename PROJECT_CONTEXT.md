...
- Evidence-first batch 29 covers Group 2 source ID 045 and Group 3 source IDs 001-003: `わすれる`, `あんないする`, `うんてんする`, and `かいものする`. All five advanced forms are released for every entry. The three `する` compounds use `できる` for their potential forms, adding 20 advanced bilingual examples in `data/katsuyou2/part-32.json`. Full ledger: `docs/KATSUYOU2_BATCH29_2026-08-12.md`.
- Evidence-first batch 30 covers Group 3 source IDs 005-008: `けっこんする`, `けんきゅうする`, `コピーする`, and `さんぽする`. All five advanced forms are released for every entry. Each `する` compound uses `できる` for its potential form, adding 20 advanced bilingual examples in `data/katsuyou2/part-33.json`. Full ledger: `docs/KATSUYOU2_BATCH30_2026-08-12.md`.
- Evidence-first batch 31 covers Group 3 source IDs 009-012: `ざんぎょうする`, `する` (do/play), `する` (put on a tie), and `しゅうりする`. Nineteen advanced bilingual examples are released in `data/katsuyou2/part-34.json`; the potential of tie-wearing `する` is deferred because it does not uniquely preserve that meaning in a standalone beginner cloze. Full ledger: `docs/KATSUYOU2_BATCH31_2026-08-12.md`.
- Evidence-first batch 32 covers Group 3 source IDs 013-016: `しゅっちょうする`, `しょうかいする`, `しょくじする`, and `しゅっせきする`. All five advanced forms are released for every entry. Each `する` compound uses `できる` for its potential form, adding 20 advanced bilingual examples in `data/katsuyou2/part-35.json`. Full ledger: `docs/KATSUYOU2_BATCH32_2026-08-12.md`.
- Evidence-first batch 33 covers Group 3 source IDs 017-020: `せつめいする`, `せんたくする`, `そうじする`, and `つれてくる`. All five advanced forms are released for every entry, adding 20 advanced bilingual examples in `data/katsuyou2/part-36.json`. The potential `つれてこられる` uses an explicit person, destination, and available day to keep the potential reading distinct from the passive; its causative example identifies all three participant roles. Full ledger: `docs/KATSUYOU2_BATCH33_2026-08-12.md`.
- Evidence-first batch 34 covers Group 3 source IDs 021-024: `でんわする`, `べんきょうする`, `もってくる`, and `よやくする`. All five advanced forms are released for every entry, adding 20 advanced bilingual examples in `data/katsuyou2/part-37.json`. The potential `もってこられる` uses an explicit object, destination, and available day to keep the potential reading distinct from the passive; its causative example identifies all three participant roles. Full ledger: `docs/KATSUYOU2_BATCH34_2026-08-12.md`.
- Evidence-first batch 35 completes Group 3 with `りゅうがくする`, then returns to the outstanding Group 1 entries `あそぶ`, existential `ある`, and `いる` meaning “need.” All five advanced forms are released for the first two entries; only past negative and conditional are released for each stative entry. This adds 14 advanced bilingual examples in `data/katsuyou2/part-38.json`. Full ledger: `docs/KATSUYOU2_BATCH35_2026-08-12.md`.
- Evidence-first batch 36 covers outstanding Group 1 source IDs 011-014: `うる`, `おく`, `おくる`, and `おす`. All five advanced forms are released for every entry, adding 20 advanced bilingual examples in `data/katsuyou2/part-39.json`. The potential forms `おける` and `おくれる` use explicit objects and locations or destinations to distinguish them from other same-sounding forms and words. Full ledger: `docs/KATSUYOU2_BATCH36_2026-08-12.md`.
- Evidence-first batch 37 completes the remaining Group 1 source IDs 015-017 and the previously missed source ID 022: `おもう`, `およぐ`, `おわる`, and time/cost `かかる`. All five advanced forms are released for the first three entries. For stative `かかる`, only past negative and conditional are released; potential is not applicable, while volitional and causative are deferred. This adds 17 advanced bilingual examples in `data/katsuyou2/part-40.json`. Full ledger: `docs/KATSUYOU2_BATCH37_2026-08-12.md`.
- The mobile quiz keeps a compact dictionary-form-to-target-form context immediately above the answer input and reduces the target card height below 720px, so opening a software keyboard does not hide the source verb.
- Progress is generated from the same source-ID and legacy composite-key matching used by the app. Run `npm run progress:update` to refresh `docs/KATSUYOU2_PROGRESS.md`, or `npm run progress:check` in validation. Full reviewed-example coverage is now 152/152 verbs (100.0%); the generated count replaces earlier manual estimates that counted recorded work rather than production-linked full coverage.
