...
2. Authentication で Google provider を有効にします。
3. Firestore Database を作成します。
4. `firebase-config.js` の `YOUR_*` を Firebase コンソールの設定値に置き換えます。
5. `verbs` コレクションに動詞データを追加します。未登録の場合は内蔵データで動きます。

Google ログインを使うため、Firebase Authentication の Authorized domains にデプロイ先ドメインが入っていることを確認してください。Firebase Hosting の標準ドメインは通常自動で入ります。
